...
import { useMonthlyBillTotal } from '@/hooks/useBills';
...
const DashboardBill = ({ status, period = 'actual' }: Props) => {
  const {
    total: bills,
    isLoading: loadingBills,
    isError: errorBills,
  } = useMonthlyBillTotal(status, period);
    ...
    <>
      <div className="flex px-4 py-3">
        ...
        <div className="flex items-center ml-3 w-11/12">
          <span className="block text-xs font-semibold">Total: {bills} €</span>
        </div>
      </div>
    </>
  );
